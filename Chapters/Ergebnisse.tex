...
Zunächst werden die Hosts mit statischen IP-Adressen konfiguriert:
...
\newline
Mit dem Kommando ``\textbf{logging synchronous}'' wird sichergestellt, dass
keine Systemmeldungen bei Befehlseingaben erscheinen.
...
\newline
R1 wird 172.17.0.1 zugewiesen. Die Clock Rate wird mit 64000 konfiguriert
und muss nur an dem Router konfiguriert werden, mit dem die
\acs{DCE}-Schnittstelle des Serial-Kabels verbunden ist (in diesem Fall
R1).\cite{cisco} Router R2 ist mit der \acs{DTE}-Schnittstelle verbunden und
ignoriert das Kommando.
\newline
\newline
\newline
...
Routern und wird der Schicht 2 des \acs{ISO}/\acs{OSI}-Referenzmodells
zugeordnet.\cite{cisco}
\newline
Es ist zu beachten, dass die Clock Rate mit 64000 konfiguriert wurde, die
Übertragungsrate allerdings bei 128 Kbit/s liegt. Es ist zu vermuten, dass
sowohl bei positiver als auch negativer Taktflanke Daten übertragen werden, was
einem impliziten Multiplikator mit dem Faktor 2 entspricht.
...
Die Schnittstelle ist nach der Konfiguration aktiviert. Der
Encapsulation Standard ist \acs{ARPA}, weclher der Schicht 2
des \acs{ISO}/\acs{OSI}-Referenzmodells
zugeordnet werden kann.\footnote{\ac{ARPA} ist der Default Encapsulation
Standard von Ethernet II bei Cisco Routern.\cite{cisco3}}
\newline
...
Windows-Eingabeaufforderung. Beide Versuche sind erfolgreich
verlaufen.\footnote{siehe Anhang unter
\ref{pingF}}
...
Den Hosts wird in diesem Versuch die jeweils letzte mögliche Host-IP-Adresse
...
konfiguriert. Das Vorgehen gestaltet sich wie im ersten Teil des Laborversuchs.
\newline
...
\newline
Auffällig sind auch die unterschiedlichen Metriken (Hop Counts). Das Netz
192.168.1.192/30 ist mit einem Hop zu erreichen. 192.168.1.128/26 hingegen
benötigt zwei Hops. Der zweite Hop stellt hier den Weg über Router 2 dar.
...
\subsubsection{Unterschiedliche RIP-Versionen konfigurieren}\label{szenario2}
...
\newline
Router 2 sendet wie bisher die Updates mittels Multicast (224.0.0.9) über beide
