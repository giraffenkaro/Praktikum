...
\subsection{Konfiguration des Routers}\label{routerconfig}
...
\subsection{Konfiguration des Switches}\label{password}
...
\begin{itemize}
  	\item Pings
  	\begin{itemize}
    	\item Switch zu Router (192.168.1.1)
    	\item Host 1 zu Switch (192.168.1.5)
    \end{itemize}
\end{itemize}
Beides verlief erfolgreich.\footnote{siehe Anhang \ref{ping1}}

\paragraph{Telnet-Session}\mbox{}\\
Ausgehend von Host 1 wird eine Telnet-Session zum Switch-Management VLAN 1
geöffnet:
...
Über das \acs{CLI} des Switches wird nun die Geschwindigkeit auf 10Mb/s bei
...
\begin{enumerate}
  	...
  	Priviledged EXEC Mode des Switches zu wechseln?
  	\begin{itemize}
...
Success rate is 100 percent (5/5), round-trip min/avg/max = 31/31/32 ms
	...
	\begin{itemize}
	  ...
	  \textit{switchport block unicast} wird ein Weiterleiten
	  verhindert.\cite{cisco4}
	  \item Dynamic Secure MAC Addresses: Diese Adressen werden zwar in der
	  MAC-Adressen-Tabelle gespeichert, beim Neustart allerdings gelöscht.
...
transparent auf OSI-Schicht 3 transparent.
\newline
...
\subsubsection{VLAN}
Durch \acp{VLAN}, die innerhalb von Switches definiert werden, können physische
Netze in mehrere logische Teilnetze gegliedert werden. Layer-3-Regeln bleiben
unberührt.
...
besteht unter anderem aus 3 Bit ``User\_priority''. Weiterhin wird 1 Bit zu
...
Listening, Learning, Forwarding. Die Zustandsänderung wird von drei Timern getriggert:
\begin{itemize}
...
diese über alle Ports. \acs{BPDU}s werden zum Austausch von
Konfigurationsnachrichten verwendet und alle zwei Sekunden gesendet. Diese
werden von jedem Switch aktualisiert und weitergeleitet.
\newline
...
Festlegung der Root-Bridge sendet nur noch diese weiterhin BPDUs.
\newline
\newline
...
\newline
Um eine schnelle Umschaltung gewährleiten zu können, werden Typen für die
...
[...]
\end{lstlisting}
Alle Ports sind zu diesem Zeitpunkt noch dem Default-VLAN 1 zugeordnet.
...
verwendet. Die Konfiguration geschieht jeweils über das CLI der
...
Adressbereiche zu den VLANs zu definieren.\cite{folien}
\newline
...
Verbindung vom Switch zum Router realisiert.
...
\subsection{Analyse der Pakete}\label{pakete}
...
Zum Zeitpunkt 0.001 empfängt PC1 auf Layer 1 einen Frame an der Schnittstelle
Fa0/1. Layer 2 enthält bei PC1 einen Ethernet Header mit Quell- und
Ziel-Adresse. Beim Switch ist auf Layer 2 ein Dot1q Header vorhanden.

\begin{figure}[ht]
  \centering
     \includegraphics[width=\linewidth]{Graphics/ethernet.PNG}
  \caption{Layer 2 zum Zeitpunkt 0.001 bei PC1}
  \label{fig:ethernet}
\end{figure}
...
\begin{figure}[ht]
  \centering
     \includegraphics[width=0.8\linewidth]{Graphics/dot1q.PNG}
  \caption{Layer 2 zum Zeitpunkt 0.001 bei WTHswitch1}
  \label{fig:dot1q}
\end{figure}

Das \ac{TCI}-Feld des Dot1q Headers beinhaltet 0x000A, was dem VLAN 10
entspricht. Der \ac{TPID} ist 0x8100, ein Standardwert zur Kennzeichnung dieses
Frames.
\newline
\newline
Der Dot1q Header bleibt bestehen bis die Anfrage bei WTHSwitch2 ankommt. Dieser
entfernt den VLAN-Tag, da der Port des Ziels sich an diesem Gerät befindet.
\newline
Bei der Antwort, dem Echo Reply, fügt WTHswitch2 das VLAN-Tag wieder hinzu,
welcher erst wieder vom WTHswitch1 entfernt wird. TPID ist in diesem Fall 0x014
und enspricht demnach dem VLAN 20.
\clearpage
Der IP-Header aus Layer 3 beinhaltet zum Zeitpunkt 0.000 folgende Informationen:
\begin{figure}[ht]
  \centering
     \includegraphics[width=\linewidth]{Graphics/ip.PNG}
  \caption{Layer 3 zum Zeitpunkt 0.001 bei PC1}
  \label{fig:ip}
\end{figure}
Sobald die Anfrage bei WHTswitch2 angelangt ist, verringert sich die \acs{TTL}
um 1.
\newline
Nachdem die Anfrage bei PC4 angekommen ist, hat der IP-Header folgende Inhalte:
\begin{figure}[ht]
  \centering
     \includegraphics[width=\linewidth]{Graphics/ip2.PNG}
  \caption{Layer 3 zum Zeitpunkt 0.005 bei PC4}
  \label{fig:ip2}
\end{figure}

Aus Quell- wird Ziel-Adresse und umgekehrt. Die TTL wird auf 128 gesetzt.
Außerdem hat sich die ID geändert.
