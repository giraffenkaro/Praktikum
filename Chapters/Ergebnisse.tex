...
quittiert wird:\footnote{Die vollständigen Frames, denen die Auschnitte
...
Nach dem erfolgreichen Aufbau der Schicht 2 können nummerierte I-Blöcke
ausgetauscht werden. Diese sind mit Zählern versehen: N(S) zählt die gesendeten
Blöcke, N(R) die empfangenen Blöcke. Sende- und Empfangszähler werden an
dieser Stelle zurückgesetzt.\cite{Siegmund2010}
\newline
Der Aufbau der Schicht 3 beginnt mit der SETUP-Nachricht und findet im
Anschluss statt. Außerdem lassen sich aus
der SETUP-Nachricht Coding Standard, der angeforderte ISDN-Dienst und die
Übertragungskapazität entnehmen:
\newline
\begin{lstlisting}
No.     Time                       Source                Destination           Protocol Info
...
Diese Nachricht dient zur Bestätigung, dass der angerufene Teilnehmer die
Verbindung angenommen hat (Hörer abgenommen). Danach wird der B-Kanal im Netz
durchgeschaltet, welcher zuvor zugewiesen wurde.\cite{Siegmund2010}\\
...
aufgeführt, da aufgrund der Versuchsanordnung nur die Nachrichten
auf Seite von Telefon 1 aufgezeichnet wurden. Es wäre auch möglich, dass die
...
Schicht-2-Adresse für das ISDN-Gerät.\cite{brandt2011net}\\
Der TEI kann entweder durch manuelle Einstellung am Gerät hinterlegt werden
...
Beim Versuch, der in den Abbildungen \ref{seq1} und \ref{seq2} dargestellt
ist, wurde zunächst der Hörer von Telefon 1 (100) abgenommen und dann die
Zielrufnummer (200) eingegeben.\\Bei der zweiten Variante wird zuerst die
Rufnummer vollständig eingegeben, bevor der Hörer abgenommen wird.
...
Wählvorgängen. Bei Einzelwahl ist die ``Called Party Number'' kein
...
In der Tabelle \ref{elemente} ist eine Übersicht gegeben, welche
...
\section{SETUP-Nachricht}\label{setup}
...
\begin{figure}[htbp]
\centering 
     \includegraphics[width=\linewidth]{Graphics/Bits.PNG}
  \caption{SETUP-Nachricht in Schicht-2-Rahmen eingebunden}
  \label{fig:setup}
\end{figure}

Die enthaltenen Informations-Elemente sind:

\begin{itemize}
  \item \textbf{Bearer Capability}
  \newline
  Die Bearer Capability spezifiziert den geforderten Übermittlungsdienst.
  \item \textbf{Calling Party Number}
  \newline
  Die Calling Party Number bezeichnet die Rufnummer des anrufenden Teilnehmers.
  \item \textbf{Called Party Number}
  \newline
  Die Ziel-Rufnummer wird im Info-Element Called Party Number gesendet.
  \item \textbf{High-Layer Compatibility}
  \newline
  Die \acs{HLC} zeigt Protokolle und Parameter an, die bei den B-Kanälen
  verwendet werden.\cite{recommendation1998digital}
\end{itemize}
