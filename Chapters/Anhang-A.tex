%********************************************************************
% Appendix
%*******************************************************

\addcontentsline{toc}{part}{Anhang}
\markright{Anhang}


\chapter{Beantwortung der Versuchsfragen}
  
\begin{enumerate}
		\item \textbf{Welche Rahmen dienen der TEI-Vergabe, und welcher TEI-Wert wird
		dem Telefon von der Vermittlung zugewiesen?}
		\newline
		Für die TEI-Vergabe dienen die Rahmen der Nachrichten ``Identity Request'' und
		''Identity Assigned''. Es handelt sich um U-Frames. Dem Telefon (Telefon 1 mit
		MSN 100) wird der TEI-Wert 64 von der Vermittlungsstelle zugewiesen.
		\newline
		\textit{siehe Anhang B: Frame 56 und 57}
		\item \textbf{Wann ist der Aufbau der Schicht 2 abgeschlossen?}
		\newline
		Nach Senden der SABME-Nachricht, welche die VSt mit einer UA-Nachricht
		beantwort, ist die Schicht 2 aufgebaut.
		\newline
		\textit{siehe Anhang B: Frame xxx}
		\item \textbf{Welchen ISDN-Dienst fordert das Telefon von der
		Vermittlungsstelle an, welche Übertragungskapazität benötigt dieser Dienst?}
		\newline
		Das Telefon fordert von der VSt den Dienst der Sprachübertragung an. Diese
		Information ist der SETUP-Nachricht zu entnehmen, ebenso wie die
		Übertragunskapazität:
		\newline
		Information transfer capability: Speech (0x00)
		\newline
		Information transfer rate: 64 kbit/s (0x10)
		\newline
		\textit{siehe Anhang B: Frame xxx}
		\item \textbf{Welche Kodierung des Sprachkanals wird gewählt?}
		\newline
		Auch die Kodierung ist der SETUP-Nachricht zu entnehmen.
		\newline
		Coding standard: ITU-T standardized coding (0x00)
		\newline
		\textit{siehe Anhang B: Frame xxx}
		\item \textbf{Welche MSN-Nummer wird in welchem Rahmen übertragen?}
		Die MSN des anrufenden Teilnehmers wird mit der SETUP-Nachrijct übertragen:
		Calling Party Number. Die Zielrufnummer wird bei Einzelwahl in den
		Informationsnachrichten übertragen.
		\newline
		\textit{(siehe Anhang B: Frame xx,xx,xx)}
		Bei einer Blockwahl wird die MSN der Zielrufnummer in der SETUP-Nachricht
		übermittelt: Called Party Number.
		\newline
		\textit{siehe Anhang B: Frame xx}
		\item \textbf{Wann ist der gesicherte Aufbau der Schicht 3 abgeschlossen?}
		\newline
		Der gesicherte Aufbau der Schicht 3 ist nach Erhalt der CONNECTION ACHNOWLEDGE
		abgeschlossen.
		\newline
		\textit{siehe Anhang B: Frame xxx}
		\item \textbf{Welchen B-Kanal weist die Vermittlung der Verbindung zu?}
		\newline
		Der Verbindung wird der B-Kanal 1 zugewiesen (B1 Channel)
		\newline
		\textit{siehe Anhang B: Frame xxx}
		\item \textbf{Wo wird die gerufene Telefonnummer übermittelt?}
		\newline
		Diese Frage wurde bereits unter 5. beantwortet.
		\item \textbf{Mit welchem Rahmen bestätigt die Vermittlung die Vollständigkeit
		der Rufnummer und beginnt den angeforderten Teilnehmer anzuwählen?}
		\newline
		Die Vollständigkeit wird durch das Senden der CALL PROCEEDING Nachricht
		bestätigt.
		\newline
		\textit{siehe Anhang B: Frame 350}
		\item \textbf{Mit welchem Rahmen signalisiert die Vermittlung, dass der
		gerufene Teilnehmeranschluss ein Endgerät besitzt, das den Sprachdienst
		erfüllen kann und ein Rufsignal aussendet?}
		\newline
		Dies geschieht mit der ALERT-Nachricht von der VSt.
		\newline
		\textit{siehe Anhang B: Frame 352}
		\item \textbf{Wann sind die Schichten 3 und 2 jeweils wieder vollständig
		abgebaut?}
		\newline
		Schicht 3 ist mit Senden des REL COM abgebaut. Nach DISC ist auch Schicht 2
		wieder abgebaut.
		\newline
		\textit{siehe Anhang B: Frame 360 und 362}
		\item \textbf{Bei ISDN gibt es die Möglichkeit, bei einem abgehenden Ruf die
		Zielrufnummer vor oder nach dem Abheben des Hörers einzugeben. Wodurch
		unterscheidet sich die Signalisierung auf Schicht 3 (SETUP- und
		INFO-Nachricht) der Teilnehmerschnittstelle in diesen beiden Fällen? Führen
		Sie dazu einen erneuten Call mit aufgelegtem Hörer (= Blockwahl) durch.}
		\newline
		Beim normalen Anruf, d.h. bei Einzelwahl ist die Zielrufnummer zum Zeitpunkt
		der SETUP-Nachricht noch nicht bekannt. Die Ziffern der Rufnummer werden im
		Anschluss einzeln in INFO-Nachrichten zur VSt gesendet bis diese vollständig
		ist. Bei Blockwahl hingegen ist die Zielrufnummer in der SETUP-Nachricht im
		Nachrichtenelement ``Called Party Number'' enthalten.
		\newline
		\textit{siehe Anhang B: Frame xxx und xxx}
\end{enumerate}

\chapter{Auszüge des Protokollmitschnitts}
